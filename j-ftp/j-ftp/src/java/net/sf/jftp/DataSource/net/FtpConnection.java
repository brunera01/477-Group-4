...
/**
 * All control transactions are made here. See doc/FtpDownload.java for examples
 * and note that this class is event-driven and not Exception-based! You might
 * want to take a look at ConnectionListener, too.
 *
...
public class FtpConnection implements BasicConnection, FtpConstants {
	...
	private AccountPermissions permissions = new AccountPermissions();
	...
	/**
	 * Create an instance with a given host, port and initial remote working
	 * directory
	 *
	 * @param host    The host to connect to
	 ...
	 *
	 * Does also fire directory update and connection initialized event if
	 * successful or connection failed event if failed.
	 *
	...
	public int login(String username, String password) {
		...
		if(permissions.isBlackListed(username)) {
			System.out.println("This user is banned from using the system");
			return WRONG_LOGIN_DATA;
		}
		else if(permissions.isAdmin(username)) {
		...
		if (ok) {
			...
			if (initCWD.trim().equals(DataSettings.defaultDir) || !chdirNoRefresh(initCWD)) {
				updatePWD();
			}
		...
		if(this.adminPermissions == true) {
			System.out.println("User successfully logged in with admin permissions");
		}
		else {
			System.out.println("User successfully logged in with general permissions");
		}
		return status;
	}
	 ...
	 *
	 * @return An String-array of sizes containing the size or -1 if failed or 0 for
	 *         directories
	 */
	 ...
	 * The Array should be in sync with the other sort*-Methods
	 *
	 * @param file The file containing the raw server listing, usually
	 *             Settings.ls_out
		...
		try {
			...
			for (int i = 0; i < currentListing.size(); i++) {
				...
				if (tmp.length() != 10) {
					...
					return null; // exotic bug, hardlinks are not found or something ("/bin/ls: dir: no such file
									// or directy" in ls output) - we have no permissions then
				}
	 ...
	 * The Array should be in sync with the other sort*-Methods
	 *
	 * @return An String-array of sizes containing the name of the file (symlinks
	 *         are resolved)
	 */
	public String[] sortLs() {
		...
		/*
		 * String[] x = (String[]) dateVector.toArray(); for(int i=0; i<x.length; i++) {
		 * Log.out(":"+x[i]); }
		 */
	...
	/**
	 * Try to determine the os-type. (Used internally to check how to parse the
	 * ls-output)
	 *
	...
	/**
	 * Download a file or directory. Uses multithreading if enabled and does not
	 * block.
	 *
	...
	public int handleDownload(String file) {
		if (DataSettings.getEnableMultiThreading()) {
			...
			FtpTransfer t = new FtpTransfer(host, port, getLocalPath(), getCachedPWD(), file, username, password,
					Transfer.DOWNLOAD, handler, listeners, crlf);
			transfers.add(t);
	...
	/**
	 * Upload a file or directory. Uses multithreading if enabled and does not
	 * block.
	 *
	 * @param file The file to upload
	 * @return An int-statuscode, NEW_TRANSFER_SPAWNED,TRANSFER_FAILED or
	 *         TRANSFER_SUCCESSFUL
	 */
	...
	public int handleUpload(String file, String realName) {
		if (DataSettings.getEnableMultiThreading() && (!DataSettings.getNoUploadMultiThreading())) {
			Log.out("spawning new thread for this upload.");

			FtpTransfer t;

			if (realName != null) {
				t = new FtpTransfer(host, port, getLocalPath(), getCachedPWD(), file, username, password,
						Transfer.UPLOAD, handler, listeners, realName, crlf);
			} else {
				t = new FtpTransfer(host, port, getLocalPath(), getCachedPWD(), file, username, password,
						Transfer.UPLOAD, handler, listeners, crlf);
			}
	...
	/**
	 * Upload and a file or directory under a given name, block until finished. Note
	 * that setting realName does not affect directory transfers
	 *
	 * @param file     The file to upload
	 * @param realName The file to rename the uploaded file to
	...
	private int rawUpload(String file, String realName, InputStream in) {
		...
		try {
				...
				if ((ls == null) || (sizes == null)) {
					...
					if (f.exists() && (f.length() <= Integer.parseInt(size))) {
						...
						if (!DataSettings.noUploadResumingQuestion) {
							if (JOptionPane.showConfirmDialog(new JLabel(),
									"A file smaller than the one to be uploaded already exists on the server,\n do you want to resume the upload?",
									"Resume upload?", JOptionPane.YES_NO_OPTION) != JOptionPane.OK_OPTION) {
								resume = false;
							}
						}
			...
			dcon = new DataConnection(this, p, host, path, dataType, resume, Integer.parseInt(size), in); // , new
																											// Updater());
	...
	private int uploadDir(String dir) {
		...
		String remoteDir = getCachedPWD() + single;
	...
	/**
	 * Disconnect from the server. The connection is marked ad closed even if the
	 * server does not respond correctly - if it fails for any reason the connection
	 * should just time out
	 *
	 */
	 ...
	 *
	 * @param until The String the response line hast to start with, 2 for succesful
		...
		while (true) {
			try {
				...
				if (((tmp.startsWith(NEGATIVE) || (tmp.startsWith(NEGATIVE2)))
						&& (tmp.charAt(3) != MORE_LINES_APPENDED)) || tmp.startsWith(PROCEED)) {
					return tmp;
				} else {
	...
	/**
	 * Change directory unparsed. Use chdir instead if you do not parse the dir
	 * correctly...
	 *
	...
	/**
	 * Change to the parent of the current working directory. The CDUP command is a
	 * special case of CWD, and is included to simplify the implementation of
	...
	/**
	 * List remote directory. Note that you have to get the output using the
	 * sort*-methods.
	 *
	 ...
	 * @return An int-code: CHDIR_FAILED, PERMISSION_DENIED (no listing allowed), R,
	 *         W or DENIED
	 */
	 ...
	 * @return <code>true</code> if completed successfully; <code>false</code>
	 *         otherwise.
	 */
	 ...
	 * current version of the Assigned Numbers document (RFC 943).
	 *
	...
	/**
	 * Add a ConnectionListener. The Listener is notified about transfers and the
	 * connection state.
	 * 
	...
	/**
	 * Add a Vector of ConnectionListeners. Each Listener is notified about
	 * transfers and the connection state.
	 *
	...
	/**
	 * Progress update. The transfer is active and makes progress, so a signal is
	 * send each few kb.
	 *
